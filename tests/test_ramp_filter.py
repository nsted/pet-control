...
class TestRampFilterBasics:
    ...
    async def test_ramp_converges_to_target(self):
        ...
        for _ in range(200):
            ...
            b._motor_state[MOTOR_ID]["pos"] = current  # simulate motor tracking command
    ...
    async def test_no_snap_after_occlusion(self):
        ...
        # Occlusion: motor stays at pos_before_occlusion for 20 ticks while
        # commands keep coming.
        for _ in range(20):
            b._motor_state[MOTOR_ID]["pos"] = pos_before_occlusion
            _seed_ramp(b, pos=pos_before_occlusion, dt_ago=dt)
            await b.send_commands([_cmd(target)])

        # First command after occlusion clears: motor resumes from physical pos.
        # The step must not exceed one ramp budget.
        b._motor_state[MOTOR_ID]["pos"] = pos_before_occlusion
        _seed_ramp(b, pos=pos_before_occlusion, dt_ago=dt)
        await b.send_commands([_cmd(target)])
    ...
    async def test_ramp_state_persists_between_calls(self):
        ...
        b._motor_state[MOTOR_ID]["pos"] = pos1  # simulate motor tracking command
...
class TestResetSeeding:
    ...
    async def test_set_hardware_zero_no_snap(self):
        ...
        b._ws = AsyncMock()
        await b.set_hardware_zero()
        b._motor_state[MOTOR_ID]["pos"] = 0.0  # simulate motor reporting new zero after hardware reset
